...
Turn on the **Remove export**policy to prohibit members of your organization from [exporting their individual vault](https://bitwarden.com/help/export-your-data/#export-an-individual-vault/). This policy is not enforced for owners and admins.
...
Turn on the **Session timeout** policy to set limits and control members' [session timeout](https://bitwarden.com/help/vault-timeout/#session-timeout/) behavior. When this policy is turned on and users edit their account's **Session timeout** settings, the **Timeout** options can not exceed the maximum you picked for the organization and some, like **On browser restart** and **Never**, will not be available. This policy does not affect organization owners.
...
Turn on the **Password generator** policy to enforce a configurable set of minimum requirements for any member-generated passwords for all members, regardless of role. Organizations can enforce:
...
Turn on the **Automatic login with SSO** policy to allow login forms to be filled and submitted automatically when accessing non-SSO apps from your identity provider. In order to enable this setting:
...
Turn on the **Remove card item type**policy to prevent members from creating or importing credit cards to organization and individual vaults.
