...
Fill assist **improves autofill accuracy on a Bitwarden-curated selection of websites**, those known to widely cause autofill issues for users, by providing human-written and reviewed instructions that your browser extension and mobile app can use to determine how to autofill credentials.
...
Fill assist can be turned on:

- For Bitwarden browser extensions, from the **Settings** → **Autofill** menu and is available for all users on version 2026.6.0 or later.
- For Bitwarden mobile apps, from the **Settings** → **Autofill**menu and is available for all users on version 2026.8.0 or later.
...
- **Substituting for default logic**: When autofilling on one of those supported sites, fill assist instructs Bitwarden to ignore the typical heuristics, including user-created [custom fields](https://bitwarden.com/help/custom-fields/), that they look in favor of a more targeted alternative.
...
Those maps are retrieved by your client on every sync or every 6 hours in order to keep up-to-date with the latest fill assist instructions. As a result, there are two key things to note:

- **Data privacy**: Once retrieved, your client stores this file locally so that **no data needs to be or is sent to Bitwarden** when an autofill action uses a fill assist.
- **Independent of client release**: Because the client recurrently retrieves the latest map, you won't need to wait for a client release in order for a new or updated instruction to take effect.
