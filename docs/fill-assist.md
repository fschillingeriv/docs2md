...
Powered by the Bitwarden [Map the Web](https://github.com/bitwarden/map-the-web/) project, fill assist is one of several tools built to resolve autofill issues. It does this by substituting the default autofill logic with human-written and reviewed instructions for how autofill should behave on any site that it supports. Let's break that down:

- **Supported sites**: Fill assist only steps in when the site you're browsing is included in [Map the Web](https://github.com/bitwarden/map-the-web/); typically, these are websites that are broadly reported to cause autofill issues for Bitwarden users.
- **Substituting for default logic**: When autofilling on one of those supported sites, fill assist instructs browser extensions to ignore the typical heuristics that they look in favor of a more targeted alternative.
...
Those maps, which can be found in the [Map the Web](https://github.com/bitwarden/map-the-web/) project, are retrieved by your browser extension client on every sync or every 6 hours in order to keep up-to-date with the latest fill assist instructions. As a result, there are two key things to note:
